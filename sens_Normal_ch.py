...
def process_excel_with_temp_data(temp_data, excel_path, sheet_name="Sens_Normal_ch"):
    ...
    for row_index, temp_value in enumerate(temp_column):
        ...
        # 채널 데이터 엑셀에 기록
        for col_index in range(8, 21):  # 열 범위: 8열~20열 (CH1~CH13)
            channel_name = f"CH {col_index - 7}"  # CH1~CH13 매칭
...
def main():
    ...
    for file_name in os.listdir(test_folder):
        if 'rx_SENS' in file_name and file_name.endswith(".txt"):
            ...
            if data is None:
                continue  # [data] 검색
            # print(data)
            # 파일 이름에서 온도와 채널 정보 추출
            temp, channel = extract_temp_channel(file_name)
            # print(f"temp{temp} channel{channel}")
            if temp in temp_data and channel:
                # 채널 데이터를 저장 (3열 데이터를 사용)
                # temp_data[temp][f"CH {channel}"] = data.iloc[:, 2].astype(float).tolist()
                column_data = data.iloc[:,2].to_numpy(dtype= float)
                temp_data[temp][f"CH {channel}"] = column_data.tolist()

    for temp, channels in temp_data.items():
        print(f"Temp {temp}: {channels}")

    # 엑셀 데이터 처리 부분
    # Excel 파일의 sheet 이름 확인
    sheet_names = pd.ExcelFile(excel_path).sheet_names
    # sheet 이름 검색
    for sheet in sheet_names:
        # print(f"- {sheet}")
        if sheet == "Sens_Normal_ch":
            process_excel_with_temp_data(temp_data, excel_path, sheet_name="Sens_Normal_ch")
